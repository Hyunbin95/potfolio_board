		...
		<table class="table table-bordered table-hover">
			...
			<c:forEach var="vo" items="${map.list}">
				<tr>
					...
					<!-- <td>&nbsp; ${vo.title}</td> -->
					<td>&nbsp; <a href="/board/detail/${vo.bno}">${vo.title}</a> <span
						class="badge bg-blue">${vo.replycnt}</span> <c:if
							test="${vo.replycnt>=5 || vo.readcnt >= 10 }">
							<img src="/userimage/hot.jpg" width="25px" height="25px" />
						</c:if></td>
					<td><img src="/userimage/${vo.image}" width="25px"
						height="25px" />&nbsp;${vo.nickname}</td>
					<td>&nbsp; ${vo.dispdate}</td>
			...
			<ul class="pagination">
				...
				<c:if test="${map.pageMaker.totalCount != 0 }">
					...
					<c:if test="${map.pageMaker.prev}">
						<li>
							<!-- 현재 출력된 시작 페이지 번호 -1을해서 데이터를 요청하는 링크를 생성 --> <a
							href="list?page=${map.pageMaker.startPage-1}&perPageNum=${map.pageMaker.cri.perPageNum}
							&searchType=${map.pageMaker.cri.searchType}&keyword=${map.pageMaker.cri.keyword}">
								이전 </a>
						</li>
					</c:if>
					<!-- 페이지 번호 출력 시작 페이지 번호부터 끝 페이지 번호까지 출력 현재 페이지 번호는 활성화 시켜서 선택할 수 없도록 -->
					<c:forEach var="idx" begin="${map.pageMaker.startPage}"
						...
						<li
							...
							&searchType=${map.pageMaker.cri.searchType}&keyword=${map.pageMaker.cri.keyword}">${idx}
						</a>
						</li>
					</c:forEach>
					<!-- 다음 출력 여부를 설정 마지막 페이지 번호 + 1 에 해당하는 링크를 설정-->
	 	...
	 	document.getElementById("searchBtn").addEventListener(
			...
			document.getElementById("count").addEventListener(
					...
					function(e) {
						...
						//데이터를 다시 호출합니다. 
						location.href = 'list?' + 'page=1&perPageNum='
								+ this.value
					})
		</script>
