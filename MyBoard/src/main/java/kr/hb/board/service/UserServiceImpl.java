...
import kr.hb.board.dao.UserDao;
...
public class UserServiceImpl implements UserService {
	...
	public int register(MultipartHttpServletRequest request) {
		...
		try {
			...
			// File 만 MulitpartFile로 받고 나머지는 전부 String
			MultipartFile image = request.getFile("image");

			// 파일을 업로드할 경로를 설정
			// 기준 webapp 또는 webcontent
			String uploadPath = request.getServletContext().getRealPath("/userimage");
			// 파일명이 중복되지 않도록 하기 위해서 랜덤한 문자열을 생성
			...
			System.out.println("회원가입 처리 오류:" + e.getMessage());
		}
		return result;
	}
	...
	public User login(HttpServletRequest request) {
		...
		try {
			...
			// System.err.println("login Service User:" + user);
	...
	public String convertAddress(String param) {
		...
		try {
			// 다운로드 받을 주소를 URL 객체로 생성
			// 파라미터에 한글이 있으면 한글을 인코딩 해야 합니다.
			...
			// 헤더 추가
			con.addRequestProperty("Authorization", "KakaoAK 87db6ec0a1298910d0c5bdec9823a0af");
			// 스트림 가져오기
			InputStreamReader inputSR = new InputStreamReader(con.getInputStream());
			...
			// 주소가 없는 경우를 대비해서 length 가 1이상인 경우만 수행
